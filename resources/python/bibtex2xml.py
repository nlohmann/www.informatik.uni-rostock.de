...
# fix editor so that it creates multiple editor, 
# split by "and"
def bibtexeditor(data):
    bibtex = '<bibtex:editor>'
    author_list = author_rex.split(data)
    if len(author_list) >1:
        bibtex = bibtex + '\n'
        for author in author_list:
...
def contentshandler(filecontents_source):
    ...
    result += '<!DOCTYPE bibtex:file PUBLIC\n'
    result += '    "-//BibTeXML//DTD XML for BibTeX v1.0//EN"\n'
    result += '    "bibtexml.dtd" >\n'
    result += '<bibtex:file xmlns:bibtex="http://bibtexml.sf.net/">\n'
    print
    for line in outdata:
        result += line + '\n'
    result += '  <!-- manual cleanup may be required... -->\n'
    result += '</bibtex:file>\n'
    return result
